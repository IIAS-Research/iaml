...
class KurtosisStatistic(Statistic):
    """[STATISTIC] Kurtosis."""

    name: str = "Kurtosis"
    _description: str = textwrap.dedent("""\
        Kurtosis measures the tailedness of numerical distributions.
        """)
    _description_long: str = textwrap.dedent("""\
        Kurtosis measures the tailedness of numerical distributions, per class for classification.
        """)
    refs: list[dict] = []
            ...
            if dataset.columns_types[col][1] not in (DataType.TEXT, DataType.SHORT_TEXT):
                for label in ['all'] + class_labels:
                    ...
                    if dataset.columns_types[col][1] == DataType.NUMERIC:
                        if label == 'all':
                            data.append(dataset.X[col].kurtosis())
                        else:
                            data.append(dataset.X.loc[dataset.y == label][col].kurtosis())
                    else:
                        data.append(None)
