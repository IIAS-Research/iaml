...
class MedianStatistic(Statistic):
    ...
    _description: str = textwrap.dedent("""\
        Median measures the central value of numerical columns.
        """)
    _description_long: str = textwrap.dedent("""\
        Median measures the central value of numerical columns, per class for classification.
        """)
    refs: list[dict] = []
            ...
            for col in dataset.X.columns:
                if dataset.columns_types[col][1] not in (DataType.TEXT, DataType.SHORT_TEXT):
                    ...
                    if dataset.columns_types[col][1] == DataType.NUMERIC:
                        data.append(dataset.X[col].median())
                    else:
                        data.append(None)
                ...
                for label in ['all'] + class_labels:
                    ...
                    if dataset.columns_types[col][1] == DataType.NUMERIC:
                        if label == 'all':
                            ...
                            data.append(dataset.X.loc[dataset.y == label][col].median())
                    else:
                        data.append(None)
        return pd.DataFrame([data], index=[str(self)], columns=columns)
