...
class ConcordanceIndexIPCWMetric(Metric):
    ...
    Weights (IPCW) for Survival Models
    """
    name = textwrap.dedent('Concordance Index with Inverse Probability of Censoring Weights (IPCW) for Survival Models')
    description = textwrap.dedent('''The Concordance Index with Inverse Probability of Censoring Weights (IPCW) 
        evaluates survival models by measuring how well they predict the order of events, 
        like survival times, while accounting for censored data. A higher index indicates better predictive accuracy.''')
    description_long = textwrap.dedent('''The Concordance Index with Inverse Probability of Censoring Weights (IPCW) 
