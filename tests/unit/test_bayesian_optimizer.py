...
class TestBayesianOptimizerDirection(unittest.TestCase):
    ...
    def assert_backend_prefers(self, optimizer, depth, score):
        ...
        self.assertAlmostEqual(result.fun, -float(score))

    def test_backend_selects_the_highest_score_and_preserves_candidate_scores(self):
        for metric, worse_score, best_score in (
            ...
            with self.subTest(metric=str(metric), scores=(worse_score, best_score)):
                ...
                self.assert_backend_prefers(optimizer, depth=3, score=best_score)
