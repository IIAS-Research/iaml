...
def _load_step():
    ...
    try:
        ...
        from iaml.actionables.normalize.act_max_abs_scaler import ActMaxAbsScaler

        return StepTestCase, ActMaxAbsScaler
    finally:
...
SKIP_REASON = None
try:
    StepTestCase, ActMaxAbsScaler = _load_step()
    ...
    ActMaxAbsScaler = None


@unittest.skipIf(SKIP_REASON is not None, SKIP_REASON)
class TestActMaxAbsScaler(StepTestCase):
    ...
    def test_transform_uses_fitted_max_abs(self) -> None:
        train = pd.DataFrame(
            {
                "age": [0, 10],
                "score": [-2.0, 4.0],
