"""Tests for ActHuberRegressor step."""
...
if SKLEARN_AVAILABLE:
    try:
        ...
        from iaml.actionables.predictors.regressor.act_huber_regressor import (
            ActHuberRegressor,
        )
    except Exception as exc:  # pragma: no cover - optional dependency guard
        _IMPORT_ERROR = exc
...
if not SKLEARN_AVAILABLE:
    _SKIP_REASON = "scikit-learn is required for ActHuberRegressor tests"
elif ActHuberRegressor is None:
    _SKIP_REASON = f"ActHuberRegressor unavailable ({_IMPORT_ERROR!r})"


@unittest.skipIf(_SKIP_REASON is not None, _SKIP_REASON)
class TestActHuberRegressor(StepTestCase):
    ...
    def test_fit_sets_model_and_scores(self) -> None:
        ...
        self.assertIsInstance(step.model, HuberRegressor)
