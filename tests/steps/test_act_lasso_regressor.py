"""Tests for ActLassoRegressor step."""
...
if SKLEARN_AVAILABLE:
    try:
        ...
        from iaml.actionables.predictors.regressor.act_lasso_regressor import (
            ActLassoRegressor,
        )
    except Exception as exc:  # pragma: no cover - optional dependency guard
        _IMPORT_ERROR = exc
...
if not SKLEARN_AVAILABLE:
    _SKIP_REASON = "scikit-learn is required for ActLassoRegressor tests"
elif ActLassoRegressor is None:
    _SKIP_REASON = f"ActLassoRegressor unavailable ({_IMPORT_ERROR!r})"


@unittest.skipIf(_SKIP_REASON is not None, _SKIP_REASON)
class TestActLassoRegressor(StepTestCase):
    ...
    def test_fit_sets_model_and_scores(self) -> None:
        ...
        self.assertIsInstance(step.model, Lasso)
