"""Tests for ActExtraSurvivalTrees step."""
import numpy as np
import pandas as pd

from .step_test_case import StepTestCase

from iaml.actionables.predictors.survival.act_extra_survival_trees import (
    ActExtraSurvivalTrees,
)


class TestActExtraSurvivalTrees(StepTestCase):
    ...
    def test_fit_sets_model_and_predicts(self) -> None:
        ...
        step = ActExtraSurvivalTrees()

        result = self.fit_step(step, dataset)

        self.assertIs(result, step)
        self.assertIsNotNone(step.model)
        self.assertEqual(step.model.__class__.__name__, "ExtraSurvivalTrees")
