"""Unit tests for VarianceStatistic."""
from __future__ import annotations

import pandas as pd

from iaml.data_type import DataType
from iaml.statistics.variance import VarianceStatistic
from tests.statistics.statistic_test_case import StatisticTestCase


class TestVarianceStatistic(StatisticTestCase):
    """Coverage for VarianceStatistic core behavior."""
    ...
    def test_classification_variances_and_columns(self) -> None:
        dataset = self.make_classification_dataset(n_samples=18, seed=25, n_classes=3)
        result = self.compute_statistic(VarianceStatistic, dataset)

        self.assertEqual(set(result.index), {"variance"})
        ...
        all_expected = dataset.X["num_2"].var()
        self.assertAlmostEqual(result.loc["variance", "num_2_all"], all_expected)

        sample_label = class_labels[0]
        label_expected = dataset.X.loc[dataset.y == sample_label, "num_2"].var()
        self.assertAlmostEqual(
            result.loc["variance", f"num_2_{sample_label}"], label_expected
        )

        for label in ["all"] + class_labels:
            self.assertTrue(pd.isna(result.loc["variance", f"cat_small_{label}"]))
            self.assertTrue(pd.isna(result.loc["variance", f"date_col_{label}"]))
