"""Unit tests for QuantileStatistic."""
from __future__ import annotations

import pandas as pd

from iaml.data_type import DataType
from iaml.statistics.quantile import QuantileStatistic
from tests.statistics.statistic_test_case import StatisticTestCase


class TestQuantileStatistic(StatisticTestCase):
    """Coverage for QuantileStatistic core behavior."""
    ...
    def test_classification_quantiles_and_columns(self) -> None:
        dataset = self.make_classification_dataset(n_samples=18, seed=23, n_classes=3)
        result = self.compute_statistic(QuantileStatistic, dataset)
        ...
        self.assertEqual(list(result.index), expected_rows)
        ...
        all_expected = dataset.X["num_2"].quantile(0.5)
        self.assertAlmostEqual(result.loc["quantile_0.5", "num_2_all"], all_expected)

        sample_label = class_labels[0]
        label_expected = dataset.X.loc[dataset.y == sample_label, "num_2"].quantile(0.5)
        self.assertAlmostEqual(
            result.loc["quantile_0.5", f"num_2_{sample_label}"],
            label_expected,
        )

        for label in ["all"] + class_labels:
            self.assertTrue(pd.isna(result.loc["quantile_0.5", f"cat_small_{label}"]))
            self.assertTrue(pd.isna(result.loc["quantile_0.5", f"date_col_{label}"]))
