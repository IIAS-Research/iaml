...
IQRStatistic = _load_module(
    "iaml.statistics.iqr_statistic",
    IAML_PATH / "statistics" / r"iqq_plot.py",
).IQRStatistic


class TestIQRStatistic(StatisticTestCase):
    """Coverage for IQRStatistic core behavior."""
    ...
    def test_classification_iqr_and_columns(self) -> None:
        dataset = self.make_classification_dataset(n_samples=18, seed=23, n_classes=3)
        result = self.compute_statistic(IQRStatistic, dataset)

        self.assertEqual(set(result.index), {"iqr"})
        ...
        series = dataset.X["num_2"]
        all_expected = series.quantile(0.75) - series.quantile(0.25)
        self.assertAlmostEqual(result.loc["iqr", "num_2_all"], all_expected)

        sample_label = class_labels[0]
        label_series = dataset.X.loc[dataset.y == sample_label, "num_2"]
        label_expected = label_series.quantile(0.75) - label_series.quantile(0.25)
        self.assertAlmostEqual(result.loc["iqr", f"num_2_{sample_label}"], label_expected)

        for label in ["all"] + class_labels:
            self.assertTrue(pd.isna(result.loc["iqr", f"cat_small_{label}"]))
            self.assertTrue(pd.isna(result.loc["iqr", f"date_col_{label}"]))
            self.assertNotIn(f"short_text_{label}", result.columns)
            self.assertNotIn(f"long_text_{label}", result.columns)

    def test_continuous_iqr_exclude_text_columns(self) -> None:
        ...
        series = dataset.X["num_2"]
        all_expected = series.quantile(0.75) - series.quantile(0.25)
        self.assertAlmostEqual(result.loc["iqr", "num_2"], all_expected)
        self.assertTrue(pd.isna(result.loc["iqr", "cat_small"]))
        self.assertTrue(pd.isna(result.loc["iqr", "date_col"]))
