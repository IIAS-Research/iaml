...
MADStatistic = _load_module(
    "iaml.statistics.mad_statistic",
    IAML_PATH / "statistics" / r"mad_statistic.py",
).MADStatistic


class TestMADStatistic(StatisticTestCase):
    """Coverage for MADStatistic core behavior."""
    ...
    def test_classification_mad_and_columns(self) -> None:
        dataset = self.make_classification_dataset(n_samples=18, seed=23, n_classes=3)
        result = self.compute_statistic(MADStatistic, dataset)

        self.assertEqual(set(result.index), {"mad"})
        ...
        all_expected = self._mad(dataset.X["num_2"])
        self.assertAlmostEqual(result.loc["mad", "num_2_all"], all_expected)

        sample_label = class_labels[0]
        label_expected = self._mad(dataset.X.loc[dataset.y == sample_label, "num_2"])
        self.assertAlmostEqual(
            result.loc["mad", f"num_2_{sample_label}"],
            label_expected,
        )

        for label in ["all"] + class_labels:
            self.assertTrue(pd.isna(result.loc["mad", f"cat_small_{label}"]))
            self.assertTrue(pd.isna(result.loc["mad", f"date_col_{label}"]))
            self.assertNotIn(f"short_text_{label}", result.columns)
            self.assertNotIn(f"long_text_{label}", result.columns)

    def test_continuous_mad_exclude_text_columns(self) -> None:
        ...
        all_expected = self._mad(dataset.X["num_2"])
        self.assertAlmostEqual(result.loc["mad", "num_2"], all_expected)
        self.assertTrue(pd.isna(result.loc["mad", "cat_small"]))
        self.assertTrue(pd.isna(result.loc["mad", "date_col"]))
